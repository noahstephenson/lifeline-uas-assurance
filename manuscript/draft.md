...
The implementation produced thirteen concrete discrepancies before publication. First, the original demo used port 8000, which was already occupied by an unrelated local service. A readiness probe could have failed or, worse, accepted a response from the wrong application. Lifeline now rejects occupied ports and considers the service ready only when the health response names the requested evidence run.
